...
public class HeyJoeSucessTest extends FlowExecutionTest {
  ...
  public void testExecuteFlow() throws InterruptedException, ExecutionException {
    ...
    FlowWorkflowActivityEntity activity = this.testSuccessExecuteFlow("5e17a4b1e638b70001fae9ca");
  ...
  public void setUp() throws IOException {
    ...
    try {
      ...
      mockServer.expect(times(1), requestTo(containsString("controller/task/execute")))
          ...
          .andExpect(jsonPath("$.taskName").value("Tail Log"))
          .andRespond(withSuccess(getMockFile("scenarios/heyjoe/mock/task1-sucess-request.json"),
              MediaType.APPLICATION_JSON));

      mockServer.expect(times(1), requestTo(containsString("controller/task/execute")))
          .andExpect(method(HttpMethod.POST))
          .andExpect(jsonPath("$.workflowName").value("HeyJoe Slack Slash Command"))
          .andExpect(jsonPath("$.taskName").value("Send Slack Log Message 1"))

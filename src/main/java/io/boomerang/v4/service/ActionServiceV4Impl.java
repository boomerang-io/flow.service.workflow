...
@Service
public class ActionServiceV4Impl implements ActionServiceV4 {
  ...
  public void actionApproval(ApprovalRequest request) {
    ...
    Optional<ActionEntity> optActionEntity = actionRepository.findById(request.getId());
    ...
    if (actionEntity.getType() == ActionType.approval) {

      String flowUserId = flowUser.getId();

      String approverGroupId = actionEntity.getApproverGroupRef();
      int numberApprovals = actionEntity.getNumberOfApprovers();
      boolean canBeApproved = false;
      //TODO: use a relationship for mapping ApproverGroup to TeamId or UserId
      //TODO: do the check to see if the ApproverGroup exists as a l 
      if (approverGroupId != null) {
        String workflowId = actionEntity.getWorkflowRef();
